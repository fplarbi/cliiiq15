    ...
    const renderMessageHeader = () => (
        <div className={clsx('flex items-center w-full,', {
            ...
            {message.dateRead && message.recipientId !== currentUserId ? (
                <span className='text-sx text-black text-italic'>(Read {timeAgo(message.dateRead)})</span>
            ) : <div></div>}
            <div className='flex'>
                <span className='text-sm font-semibold text-gray-900'>{message.senderName}</span>
                <span className='text-sm text-gray-600'>{message.created}</span>
            </div>
        </div>
    ...
    return (
        <div className='grid grid-rows-1'>
            <div className={clsx('flex gap-2 mb-3', {
                ...
                {!isCurrentUserSender && renderAvartar()}
